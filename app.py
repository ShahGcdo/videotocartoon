...
# Streamlit Config
st.set_page_config(page_title="🎨 Optimized Video App", layout="centered")
st.title("🎨 AI Video Effects (Optimized)")
...
def add_rain_effect(frame, density=0.002):
    ...
    for _ in range(int(h * w * density)):
        x, y = random.randint(0, w - 1), random.randint(0, h - 20)
        length, thickness = random.randint(10, 20), 1
        color = (200, 200, 255)
...
def apply_watermark(input_path, output_path, text="@USMIKASHMIRI"):
    ...
    cmd = [
        "ffmpeg", "-y", "-i", input_path, "-vf", watermark_filter,
        "-c:v", "libx264", "-preset", "fast", "-crf", "22", "-pix_fmt", "yuv420p", output_path
    ]
    subprocess.run(cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)

# --- UI ---
st.header("📤 Upload Video and Apply Style")
file = st.file_uploader("Choose a video", type=["mp4"])
style = st.selectbox("Choose Style", ["None", "🌸 Soft Pastel Anime-Like Style"])
add_rain = st.checkbox("Add Light Rain")
add_wm = st.checkbox("Add Watermark")

if file and st.button("Process Video"):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        def process_frame(frame):
            ...
            if add_rain:
                frame = add_rain_effect(frame, density=0.002)
            return frame

        # Chunking
        chunk_size = 10
        chunks = []
        for i in range(0, int(clip.duration), chunk_size):
            sub = clip.subclip(i, min(i + chunk_size, clip.duration))
            sub_styled = sub.fl_image(process_frame)
            chunks.append(sub_styled)
        ...
        if add_wm:
            ...
            apply_watermark(processed_path, wm_path)
            final_path = wm_path
        else:
        ...
        with open(final_path, "rb") as f:
            st.download_button("⬇️ Download Final", f, file_name="styled_output.mp4")
